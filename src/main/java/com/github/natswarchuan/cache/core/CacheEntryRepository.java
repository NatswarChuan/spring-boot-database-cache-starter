...
public interface CacheEntryRepository extends JpaRepository<CacheEntry, Long> {
    ...
    /**
     * Phương thức 'Update or Insert' (Upsert) tùy chỉnh.
     * Nó tìm một entry. Nếu tồn tại, nó cập nhật 'value'.
     * Nếu không, nó tạo một entry mới.
     * Sử dụng @Transactional để đảm bảo tính toàn vẹn.
     */
    @Transactional
    default void upsert(String cacheName, String cacheKey, byte[] value) {
        CacheEntry entry = findByCacheNameAndCacheKey(cacheName, cacheKey)
                .orElse(new CacheEntry()); // Tạo mới nếu không tìm thấy
        
        entry.setCacheName(cacheName);
        entry.setCacheKey(cacheKey);
        entry.setValue(value);
        
        save(entry); // Lưu (insert hoặc update)
    }
}
