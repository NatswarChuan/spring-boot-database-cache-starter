...
/**
 * Triển khai interface Cache của Spring.
 * Lớp này chứa logic cốt lõi cho get, put, evict.
 */
public class DatabaseCache implements Cache {
    ...
    public ValueWrapper get(Object key) {
        ...
        return repository.findByCacheNameAndCacheKey(name, keyStr)
                .map(entry -> {
    ...
    public <T> T get(Object key, Callable<T> valueLoader) {
        ...
        // Đồng bộ hóa trên key để tránh "thundering herd"
        synchronized (this) {
            wrapper = get(key); // Kiểm tra lại
            if (wrapper != null) {
                @SuppressWarnings("unchecked")
                T value = (T) wrapper.get();
                return value;
            ...
            this.put(key, value); // Lưu vào cache sau khi tải
            return value;
        }
    }
    ...
    private byte[] serialize(@Nullable Object obj) {
        ...
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream();
            ...
            throw new CacheSerializationException("Lỗi khi serialize cache value cho key '" + obj + "'", e);
        }
    }
